...
import { getDoctorKycStatus, submitDoctorKyc } from "@/lib/endpoints";
...
export default function DoctorKycPage() {
  ...
  useEffect(() => {
    ...
    getDoctorKycStatus()
      .then((res) => {
        setStatus(res);
        if (res.kyc_verified) router.replace("/doctor");
      })
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await submitDoctorKyc(formData);
      setStatus(res);
    } catch (err) {
        ...
        <div className={styles.card}>
          {status?.kyc_status === "pending" ? (
            ...
            <>
              ...
              <p className={styles.subtitle}>
                Before accessing patient records, we need a quick selfie and a photo of your ID.
              </p>
