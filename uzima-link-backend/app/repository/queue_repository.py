...
def create_queue_entry(db: Session, patient_id: UUID, facility_id: UUID) -> models.QueueEntry:
...
def get_queue_for_facility(db: Session, facility_id: UUID, status: str = None):
    query = db.query(models.QueueEntry).filter(models.QueueEntry.facility_id == facility_id)
    if status:
...
def assign_doctor(db: Session, entry: models.QueueEntry, doctor_id: UUID) -> models.QueueEntry:
...
def update_status(db: Session, entry: models.QueueEntry, status: str) -> models.QueueEntry:
    entry.status = status
    entry.updated_at = datetime.utcnow()
    db.commit()
    db.refresh(entry)
