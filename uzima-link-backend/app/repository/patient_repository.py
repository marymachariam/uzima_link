...
def get_patient_by_system_uid(db: Session, system_uid: str) -> models.Patient | None:
    return (
        db.query(models.Patient).filter(models.Patient.system_uid == system_uid).first()
    )
...
def mark_patient_kyc_verified(
    db: Session, patient: models.Patient, job_id: str
) -> models.Patient:
...
def submit_kyc(
    db: Session, patient: models.Patient, selfie_url: str, id_document_url: str
) -> models.Patient:
...
def get_pending_kyc_patients(db: Session, limit: int = 50):
    return (
        db.query(models.Patient)
        .filter(models.Patient.kyc_status == "pending")
        .limit(limit)
        .all()
    )
...
def resolve_kyc(
    db: Session, patient: models.Patient, approve: bool, note: str = None
) -> models.Patient:
    from datetime import datetime
