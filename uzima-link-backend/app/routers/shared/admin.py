...
from app.repository import facility_repository, patient_repository, user_repository
...
@router.post(
    "/facilities/{facility_id}/generate-invite-code", response_model=schemas.FacilityOut
)
...
def invite_staff(
    ...
    if data.facility_kmhfr_code:
        facility = facility_repository.get_facility_by_kmhfr_code(
            db, data.facility_kmhfr_code
        )
    elif data.facility_registration_number:
        facility = facility_repository.get_facility_by_registration_number(
            db, data.facility_registration_number
        )

    if not facility:
        raise HTTPException(
            status_code=404, detail="No facility found with that identifier"
        )
    ...
    try:
        email_service.send_staff_invite_email(
            data.email, data.full_name, data.role, facility.name, facility.invite_code
        )
    except Exception as e:
        print(f"Failed to send staff invite email: {e}")
        raise HTTPException(status_code=500, detail="Failed to send invite email")

    return schemas.StaffInviteOut(
        message=f"Invite sent to {data.email}", facility_name=facility.name
    )
...
@router.post(
    "/kyc/patients/{patient_id}/decision", response_model=schemas.AdminPatientKycOut
)
def decide_patient_kyc(
    ...
    updated = patient_repository.resolve_kyc(
        db, patient, approve=data.approve, note=data.note
    )
    ...
    if data.approve:
        ...
        if user:
            try:
                email_service.send_patient_kyc_approved_email(
                    user.email, patient.full_name
                )
            except Exception as e:
...
def decide_doctor_kyc(
    ...
    updated = user_repository.resolve_doctor_kyc(
        db, user, approve=data.approve, note=data.note
    )
