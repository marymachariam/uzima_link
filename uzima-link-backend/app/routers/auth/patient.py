...
def register_patient(request: Request, data: schemas.PatientRegister, db: Session = Depends(get_db)):
    ...
    try:
        new_user = user_repository.create_user(
            db, data.email, auth_service.hash_password(data.password), "patient",
            patient_id=new_patient.id, is_verified=False
        )
    except Exception:
        db.rollback()
    ...
    audit_service.log_action(db, user_id=new_user.id, action="register", resource_type="patient", resource_id=new_patient.id)
...
def start_login(request: Request, data: schemas.PatientLoginStart, db: Session = Depends(get_db)):
...
@limiter.limit("10/hour")
def verify_login(request: Request, data: schemas.PatientLoginVerify, db: Session = Depends(get_db)):
    ...
    audit_service.log_action(db, user_id=user.id, action="login", resource_type="user", resource_id=user.id)

    token = auth_service.create_access_token({
        "user_id": user.id, "role": "patient", "patient_id": user.patient_id, "verified": True
    })
...
def forgot_password(request: Request, data: schemas.ForgotPasswordRequest, db: Session = Depends(get_db)):
    ...
    if user and user.role == "patient":
        ...
        try:
            email_service.send_password_reset_email(user.email, reset_token, to_name=display_name)
        except Exception as e:
...
def reset_password(data: schemas.ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    audit_service.log_action(db, user_id=user.id, action="password_reset", resource_type="user", resource_id=user.id)
    try:
