...
from app.services import audit_service, transcription_service
...
def record_consultation(
    ...
    if not consent_repository.has_doctor_access(
        db, patient_id, user.id, user.facility_id
    ):
        raise HTTPException(
    ...
    try:
        ...
        swahili_transcript = transcription_service.translate_text(
            english_transcript, target_language="Swahili"
        )
    except Exception:
        raise HTTPException(
            status_code=502, detail="Failed to process audio. Please try again."
        )
    ...
    visit_repository.add_transcripts(
        db,
        visit,
        english_transcript=english_transcript,
        swahili_transcript=swahili_transcript,
    )
